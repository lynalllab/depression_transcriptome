...
mostafavi_tt_ncc_pooled <- topTable(voom_fit, coef = "DisorderMDD", number = nrow(data_filtered))
...
mostafavi_tt_ncc_m <- topTable(voom_fit_m, coef = "DisorderMDD", number = nrow(data_filtered_male))
...
mostafavi_tt_ncc_f <- topTable(voom_fit_f, coef = "DisorderMDD", number = nrow(data_filtered_female))
...
```{r}
cellchoice_broad <- c("B-cells", "CD4+ T-cells", "CD8+ T-cells", "Monocytes", "NK cells", "Eosinophils", "Neutrophils")
...
toselect <- apply(tmp,1,function(x){sum(x<0.1)}>=268)
toselect %>% table 
...
final_metadata_fem_cc <- final_metadata_cc %>% filter(Sex == "F")
final_metadata_male_cc <- final_metadata_cc %>% filter(Sex == "M")
```
...
```{r}
design_pooled_cc <- model.matrix(~ Disorder + Batch + Sex + Age + `B-cells` + `CD4+ T-cells` + `CD8+ T-cells` + Monocytes + `NK cells`+ Eosinophils + Neutrophils, data = final_metadata_cc)

design_female_cc <- model.matrix(~ Disorder + Batch + Age + `B-cells` + `CD4+ T-cells` + `CD8+ T-cells` + Monocytes + `NK cells`+ Eosinophils + Neutrophils, data = final_metadata_fem_cc)

design_male_cc <- model.matrix(~ Disorder + Batch + Age + `B-cells` + `CD4+ T-cells` + `CD8+ T-cells` + Monocytes + `NK cells` + Eosinophils + Neutrophils, data = final_metadata_male_cc)
```
...
mostafavi_tt_cc_pooled <- topTable(voom_fit_cc, coef = "DisorderMDD", number = nrow(data_filtered_cc))
...
mostafavi_tt_cc_m <- topTable(voom_fit_m_cc, coef = "DisorderMDD", number = nrow(data_filtered_male_cc))
...
mostafavi_tt_cc_f <- topTable(voom_fit_f_cc, coef = "DisorderMDD", number = nrow(data_filtered_female_cc))
...
```{r}
catgs <- final_metadata_fem_cc %>% dplyr::select(sample_id_match, Disorder, Batch, Age, `B-cells`:Neutrophils) 
catgs[, c(2,3)] <- lapply(catgs[, c(2,3)], factor)
design = model.matrix(~ Batch + Age + `B-cells` + `CD4+ T-cells` + `CD8+ T-cells` + Monocytes + `NK cells` + Eosinophils + Neutrophils, data = catgs)
mm <- model.matrix(~Disorder, data = catgs)
batch_correct_m_f <- removeBatchEffect(data_normalised_female_cc, covariates = design[,-1], design = mm)

catgs <- final_metadata_male_cc %>% dplyr::select(sample_id_match, Disorder, Batch, Age, `B-cells`:Neutrophils) 
catgs[, c(2,3)] <- lapply(catgs[, c(2,3)], factor)
design = model.matrix(~ Batch + Age + `B-cells` + `CD4+ T-cells` + `CD8+ T-cells` + Monocytes + `NK cells` + Eosinophils + Neutrophils, data = catgs)
mm <- model.matrix(~Disorder, data = catgs)
batch_correct_m_m <- removeBatchEffect(data_normalised_male_cc, covariates = design[,-1], design = mm)

catgs <- final_metadata_cc %>% dplyr::select(sample_id_match, Disorder, Batch, Sex, Age, `B-cells`:Neutrophils) 
catgs[, c(2,3)] <- lapply(catgs[, c(2,3)], factor)
design = model.matrix(~ Batch + Sex + Age + `B-cells` + `CD4+ T-cells` + `CD8+ T-cells` + Monocytes + `NK cells` + Eosinophils + Neutrophils, data = catgs)
mm <- model.matrix(~Disorder, data = catgs)
batch_correct_m_pooled <- removeBatchEffect(data_normalised_male_cc, covariates = design[,-1], design = mm)

metd_m <- final_metadata_cc

save(metd_m, batch_correct_m_f, batch_correct_m_m, file = "../for_WGCNA/mostafavi.Rdata")
```
...
```{r}
metd_sen <- final_metadata_cc %>% filter(is.na(BMI) == F)
metd_sen_f <- metd_sen %>% filter(Sex == "Female")
metd_sen_m <- metd_sen %>% filter(Sex == "Male")
  
data_filtered_sens <- data_filtered$counts
data_filtered_sens_f <- data_filtered_female$counts
data_filtered_sens_m <- data_filtered_male$counts
...
```{r}
design_sens <- model.matrix(~ Disorder + Batch + Age + Sex + BMI + `B-cells` + `CD4+ T-cells` + `CD8+ T-cells` + Monocytes + `NK cells` + Eosinophils + Neutrophils, data = metd_sen)

design_sens_m <- model.matrix(~ Disorder + Batch + Age + BMI + `B-cells` + `CD4+ T-cells` + `CD8+ T-cells` + Monocytes + `NK cells` + Eosinophils + Neutrophils, data = metd_sen_m)

design_sens_f <- model.matrix(~ Disorder + Batch + Age + BMI + `B-cells` + `CD4+ T-cells` + `CD8+ T-cells` + Monocytes + `NK cells` + Eosinophils + Neutrophils, data = metd_sen_f)
...
mostafavi_tt_sens_pooled <- topTable(voom_fit_sens, coef = "DisorderMDD", number = nrow(data_filtered_sens))
...
mostafavi_tt_sens_m <- topTable(voom_fit_m_sens, coef = "DisorderMDD", number = nrow(data_filtered_sens_m))
...
mostafavi_tt_sens_f <- topTable(voom_fit_f_sens, coef = "DisorderMDD", number = nrow(data_filtered_sens_f))
...
```{r}
final_metadata_p <- merge(final_metadata, cc_broad_all, by.x = "sample_id_match", by.y = "X")
design <- model.matrix(~ Disorder + Batch + Sex + Age + `B-cells` + `CD4+ T-cells` + `CD8+ T-cells` + Monocytes + `NK cells`+ Eosinophils + Neutrophils + Platelets, data = final_metadata_p)
design_male <- filter_metadata_p %>% filter(Sex == "Male") %>% model.matrix(~ Disorder + Batch + Age + `B-cells` + `CD4+ T-cells` + `CD8+ T-cells` + Monocytes + `NK cells` + Eosinophils + Neutrophils + Platelets, data = .)
design_female <- filter_metadata_p %>% filter(Sex == "Female") %>% model.matrix(~ Disorder + Batch + Age + `B-cells` + `CD4+ T-cells` + `CD8+ T-cells` + Monocytes + `NK cells`+ Eosinophils + Neutrophils + Platelets, data = .)

```

Limma voom for heterogeneity analysis

```{r}
voom_pooled_sens_p <- voomWithQualityWeights(data_filtered_cc, design = design, plot = T) #tmm normalised values from edgeR run
voom_fit_sens_p <- lmFit(voom_pooled_sens_p, design = design)
voom_fit_sens_p <- eBayes(voom_fit_sens_p) #either trend = T or voom use they are equivalent from manual
mostafavi_tt_sens_p_pooled <- topTable(voom_fit_sens_p, coef = "DisorderMDD", number = 18277)

voom_m_sens_p <- voomWithQualityWeights(data_filtered_male_cc, design = design_male, plot = T) #tmm normalised values from edgeR run
voom_fit_m_sens_p <- lmFit(voom_m_sens_p, design = design_male)
voom_fit_m_sens_p <- eBayes(voom_fit_m_sens_p) #either trend = T or voom use
mostafavi_tt_sens_p_m <- topTable(voom_fit_m_sens_p, coef = "DisorderMDD", number = 18731)

voom_f_sens_p <- voomWithQualityWeights(data_filtered_female_cc, design = design_female, plot = T) #tmm normalised values from edgeR run
voom_fit_f_sens_p <- lmFit(voom_fit_f_sens_p, design = design_female)
voom_fit_f_sens_p <- eBayes(voom_fit_f_sens_p) #either trend = T or voom use
mostafavi_tt_sens_p_f <- topTable(voom_fit_f_sens_p, coef = "DisorderMDD", number = 18543)
